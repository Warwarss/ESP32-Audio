...
void i2s_transmit_wav_task(void *pvParameters) {
    ...
    while (total_sent_bytes < data_size) {
        ...
        if (!start_mixing) {
            switch (bits_per_sample) {
                printf("Transmitting %u bit data\n", bits_per_sample);
                case 16: {
                    ...
                    for (int i = 0; i < num_samples; i++) {
                        int32_t sample = (int32_t)((0x00) |
                                                    (current_pos[i*2] << 8) |
                                                    (current_pos[i*2 + 1] << 16));      
                        current_pos_16[i*3] = sample & 0xFF; // Padding for 24-bit
                        current_pos_16[i*3 + 1] = (sample >> 8) & 0xFF;
                        current_pos_16[i*3 + 2] = (sample >> 16) & 0xFF;
                    }
